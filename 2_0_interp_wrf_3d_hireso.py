...
#dates and time for eruption
date = "2026-07-09" #date of windfile/eruption
date_slash = date.replace("-", "/")
date_clear = date.replace("-", "")
start_hour = 7 # eruption start time

#main directory
main_dir = Path(__file__).resolve().parents[0]

# directory to import and save
direc1 = f"{main_dir}/{date_clear}_{start_hour:02d}0000_files/intp/" #output
dir_app = f"{main_dir}/{date_clear}_{start_hour:02d}0000_files/extracted/" #input
...
def check_data(windstart_utc):
    raw_nc = nc.Dataset(dir_app + windstart_utc.strftime("%Y-%m-%d_%H%M%S_raw.nc"), "r")
    start_utc = dt.datetime.strptime(raw_nc.variables["XTIME"].description[-19:], "%Y-%m-%d %H:%M:%S") + dt.timedelta(
        minutes=int(raw_nc.variables["XTIME"][0]))
    if windstart_utc != start_utc:
        print(windstart_utc, start_utc)
        return
    else:
        return
...
def interp_wrf_3d(windstart_utc):
    raw_nc = nc.Dataset(dir_app + windstart_utc.strftime("%Y-%m-%d_%H%M%S_raw.nc"), "r")
    time_interval = int(raw_nc.variables["XTIME"][1] - raw_nc.variables["XTIME"][0])  # min
    time_slice = len(raw_nc.variables["XTIME"])
    time_range = time_slice * time_interval  # min
    ...
    max_iz = np.arange(49)[[np.min(z_asl_raw[:, i, :, :]) < alt_intp[-1] for i in range(49)]][-1] + 1
    ...
    print(np.max(lon_raw), np.min(lon_raw))
    print(np.max(lat_raw), np.min(lat_raw))
    ...
    qv_raw = np.array(raw_nc.variables["QVAPOR"][:time_slice, :max_iz, r_ns_raw[0]:r_ns_raw[1],
                      r_ew_raw[0]:r_ew_raw[1]])  # ファイルによってQVだったりQVAPORだったりする
    p_raw = (np.array(
    ...
    for t in range(time_slice):
        ...
        def intp_layer_t_pot(iz):
            t_pot_iz = t_pot_raw[t, iz, :, :]
            t_pot_intp_iz_griddata = griddata(points=pts, values=t_pot_iz.reshape(-1), xi=(xx, yy), method=intp_method)
            return t_pot_intp_iz_griddata.T

        def intp_layer_p(iz):
            p_iz = p_raw[t, iz, :, :]
            p_intp_iz_griddata = griddata(points=pts, values=p_iz.reshape(-1), xi=(xx, yy), method=intp_method)
            return p_intp_iz_griddata.T
        ...
        t_pot_intp_t = np.stack(list(map(intp_layer_t_pot, range(max_iz))))
...
# interpolate in z axis
        ...
        dat = v_intp_t
        v_intp0 = np.stack(list(map(intp_grid_dat, r_yy.reshape(-1), r_xx.reshape(-1))))
        v_intp1 = v_intp0.T.reshape(1, len(alt_intp), len(lat_intp), len(lon_intp))
        dat = w_intp_t
        w_intp0 = np.stack(list(map(intp_grid_dat, r_yy.reshape(-1), r_xx.reshape(-1))))
        w_intp1 = w_intp0.T.reshape(1, len(alt_intp), len(lat_intp), len(lon_intp))
        dat = rho_intp_t
        rho_intp0 = np.stack(list(map(intp_grid_dat, r_yy.reshape(-1), r_xx.reshape(-1))))
        rho_intp1 = rho_intp0.T.reshape(1, len(alt_intp), len(lat_intp), len(lon_intp))
        dat = t_pot_intp_t
        t_pot_intp0 = np.stack(list(map(intp_grid_dat, r_yy.reshape(-1), r_xx.reshape(-1))))
        t_pot_intp1 = t_pot_intp0.T.reshape(1, len(alt_intp), len(lat_intp), len(lon_intp))
        dat = p_intp_t
        p_intp0 = np.stack(list(map(intp_grid_dat, r_yy.reshape(-1), r_xx.reshape(-1))))
        p_intp1 = p_intp0.T.reshape(1, len(alt_intp), len(lat_intp), len(lon_intp))
        ...
        if t == 0:
            ...
            t_pot_intp = np.concatenate([t_pot_intp, t_pot_intp1])
            p_intp = np.concatenate([p_intp, p_intp1])

    # export as a netCDF file
    intp_nc = nc.Dataset(f"{direc1}/0_{windstart_utc.strftime('%Y-%m-%d_%H%M%S')}_intp.nc", "w", format="NETCDF3_CLASSIC") #changed
    ...
    intp_nc_t_pot = intp_nc.createVariable("t_pot", np.dtype("float32").char, ("time", "alt", "lat", "lon"))
    intp_nc_t_pot.long_name = "potential temperature"
    intp_nc_t_pot.units = "K"

    intp_nc_p = intp_nc.createVariable("p", np.dtype("float32").char, ("time", "alt", "lat", "lon"))
    ...
    intp_nc_t_pot[:] = t_pot_intp
    intp_nc_p[:] = p_intp

    intp_nc.close()


interp_wrf_3d(pd.to_datetime(f"{date_slash} {start_hour:02d}:00")) #change depending on the date
