...
#dates and time for eruption (change date and time when needed)
date = "2026-07-09" #date of windfile/eruption
date_slash = date.replace("-", "/")
date_clear = date.replace("-", "")
start_hour = 7 # eruption start time

dir1 = f"{main_dir}/{date_clear}_{start_hour:02d}0000_files_v1/"  # the directory where the output trajectory file is saved
dir_app =  f"{main_dir}/{date_clear}_{start_hour:02d}0000_files/intp/"  # the directory where the wind and air density data are located
# mapping dem
...
def rise(erno):
    ...
    windstart_utc = pd.to_datetime(f"{date_slash} {start_hour:02d}:00")  # 9 hours: JST - UTC
    ...
    winddat = nc.Dataset(dir_app + f"0_{date}_{start_hour:02d}0000_intp.nc", "r")
...
def traj(erno):
    ...
    windstart_utc = pd.to_datetime(f"{date_slash} {start_hour:02d}:00")
    winddat = nc.Dataset(dir_app + f"0_{date}_{start_hour:02d}0000_intp.nc", "r")
