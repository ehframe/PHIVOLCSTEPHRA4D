...
outpath = main_dir / f"{date_clear}_{start_hour:02d}0000_files" / "Load in grids (vertical)"
...
TIME_WINDOW = 360.0  # Active particle window (seconds)
...
for file_path in csvs:
    ...
    print(f'generating vertical load for: {d_name}')
    ...
    title_text = ax.set_title(
        f"Vertical Ash Propagation ({d_name}) — Time: {int(time_steps[0])}s",
        fontsize=12,
        fontweight="bold",
    )
    ...
    def update(frame):
        ...
        if np.any(time_mask):
            dist_c_arr = dist_from_vent[time_mask]
        ...
        title_text.set_text(
            f"Vertical Ash Propagation ({d_name}) — Time: {int(current_time)}s"
        )
    ...
    gif_file = outpath / f"height_{d_name}_ash_propagation.gif"
