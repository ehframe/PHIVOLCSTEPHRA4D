...
# -------------------------------------------------------------------
# 4. COMPUTE DISPERSION GRID FRAMES (PHYSICAL KM ASL)
# -------------------------------------------------------------------
def compute_ash_grid(current_time):
    ...
    # Physical kilometer scaling: starts at Vent Summit (2.435 km) up to Plume Top Ceiling (12.0 km)
    z_height_km = np.where(
        masked_grid > 0,
        VENT_ELEV_KM
        + (np.log10(masked_grid + 1.0) / np.log10(11.0))
        * (MAX_PLUME_TOP_KM - VENT_ELEV_KM),
        np.nan,  # NaN keeps zero-concentration regions fully transparent
    )
...
html_file = outpath / "total_composite_ash_propagation_3d_v2.html"
