...
# Coordinate Transformers (UTM Zone 51N <-> WGS84 Lat/Lon)
...
# Expanded spatial domain bounds for the plume grid
llcrnlon, urcrnrlon = 122.800842, 124.105784
llcrnlat, urcrnrlat  = 10.199636, 10.690102
"""llcrnlon, urcrnrlon = 122.000000, 125.000000
llcrnlat, urcrnrlat = 9.500000, 11.200000"""

# Increased plume calculation grid points (280 x 200)
...
# -------------------------------------------------------------------
# DISCRETE COLORBAR & LEVEL MATCHING
# -------------------------------------------------------------------
...
# ===================================================================
# SECTION 3a: TERRAIN DEM & BASEMAP
# ===================================================================
...
def fetch_terrain_grid_km(lon_grid, lat_grid, zoom=10):
    ...
    stitched_img = Image.new(
        "RGB", ((x_max - x_min + 1) * 256, (y_end - y_start + 1) * 256)
    )
...
terrain_grid_correct = (
    np.flipud(terrain_z_km) if Y_lat[0, 0] > Y_lat[-1, 0] else terrain_z_km
)
...
# ===================================================================
# SECTION 4: EXPANDED PLUME GRID CALCULATION
# ===================================================================
def compute_atmospheric_plume(current_time):
    ...
    if np.any(time_mask):
        ...
        for xc, yc, zc, t in zip(xc_arr, yc_arr, zc_arr, t_arr):
            ...
            # Evaluates plume center against the entire enlarged grid mesh (X_m, Y_m)
            dx, dy = X_m - xc, Y_m - yc
            nakami_exp = -(dx**2 + dy**2) / sigma

            # Expanded threshold (-60 instead of -40) to capture fainter plume edges across the grid
            if np.max(nakami_exp) > -60:
                weight = np.where(
                    nakami_exp > -60,
                    (1.0 / (sigma * np.pi)) * np.exp(nakami_exp),
                    0.0,
                )
                ash_grid += weight
...
# Integrate ash load across the expanded plume evaluation grid
dx = np.abs(X_m[0, 1] - X_m[0, 0])
dy = np.abs(Y_m[1, 0] - Y_m[0, 0])
cell_area_m2 = dx * dy

# Sum across all active plume grid cells
...
# 2. Monitoring Sites from CSV
sites_csv = main_dir / f"{date_clear}_{start_hour:02d}0000_files" / "mapoutputs" / "totalload.csv"

if sites_csv.exists():
    print(f"Reading site load data from {sites_csv}...")
    ...
    if lat_col and lon_col:
        ...
        for idx, row in df_sites.iterrows():
            ...
            if (llcrnlat <= s_lat <= urcrnrlat) and (llcrnlon <= s_lon <= urcrnrlon):
                ...
                print(f"Skipping {s_name}: Position out of bounds.")

        print(f"Successfully loaded {loaded_count} sites.")
    else:
        print(f"ERROR: Lat/Lon columns missing. Present: {list(df_sites.columns)}")
else:
    print(f"ERROR: File NOT found at {sites_csv}")
...
# 3. Atmospheric Ash Load Surface
fig.add_trace(
    go.Surface(
        x=X_lon,
        y=Y_lat,
...
# 4. Central Eruption Column Mesh
...
html_file = outpath / "ash_composite_v5.html"
fig.write_html(html_file)
print(f"-> Saved updated 3D plot to: {html_file}")