...
# ===================================================================
# SECTION 4: ORIGINAL PLUME PROPAGATION & GROUND LOAD ENGINE
# ===================================================================
def compute_atmospheric_and_ground_load(current_time):
    time_mask = all_t <= current_time
    plume_grid = np.zeros_like(X_m)
    ground_grid = np.zeros_like(X_m)
    ...
    if np.any(time_mask):
        ...
        # Original Plume Dispersion Formulation
        sigma = np.maximum(
        ...
        for xc, yc, zc, sig, td in zip(xc_arr, yc_arr, zc_arr, sigma, t_diff):
            ...
            if np.max(nakami_exp) > -60:
                base_kernel = np.where(
                    nakami_exp > -60,
                    (1.0 / (sig * np.pi)) * np.exp(nakami_exp),
                    0.0,
                )

                # Original airborne plume accumulation
                plume_grid += base_kernel
                height_grid += base_kernel * zc
                weight_sum += base_kernel

                # Accumulate deposited ground fall onto DEM footprint based on particle settling velocity
                c_lon_pt, c_lat_pt = utm_to_latlon.transform(xc, yc)
                local_dem = float(terrain_interp([c_lat_pt, c_lon_pt])[0])

                current_z_all = zc - (td * V_SETTLE_SPECTRUM_KM_S)
                deposited_mask = current_z_all <= local_dem

                if np.any(deposited_mask):
                    deposited_weight = np.sum(deposited_mask) * WEIGHT_PER_BIN
                    ground_grid += base_kernel * deposited_weight

    plume_load_g = plume_grid * 1000.0
    ground_load_g = ground_grid * 1000.0

    masked_plume = np.where(plume_load_g >= levels[0], plume_load_g, np.nan)
    masked_ground = np.where(ground_load_g >= levels[0], ground_load_g, np.nan)

    with np.errstate(divide="ignore", invalid="ignore"):
        avg_z = np.where(weight_sum > 0, height_grid / weight_sum, np.nan)

    plume_z = np.where(
        ~np.isnan(masked_plume),
        np.maximum(avg_z, ground_footprint_z + 0.05),
        np.nan,
    )

    return masked_plume, plume_z, masked_ground, ground_load_g
...
plume_frames_data = [compute_atmospheric_and_ground_load(t) for t in time_steps]

# ===================================================================
# SECTION 5: SITE MARKERS & DYNAMIC LOAD INTEGRATION
# ===================================================================
        ...
        for idx, row in df_sites.iterrows():
            ...
            if (llcrnlat <= s_lat <= urcrnrlat) and (llcrnlon <= s_lon <= urcrnrlon):
                ...
                sites_info.append({
                    ...
                    "lat": s_lat,
                    "lon": s_lon,
                    "elev": site_ground_elev,
                    "csv_load": csv_load_val,
                    "is_vent": False
                })

def build_site_trace(ground_load_full_grid):
    site_load_interp = RegularGridInterpolator(
        (lat_axis_sorted, lon_axis_sorted),
        ground_load_full_grid,
        bounds_error=False,
        fill_value=0.0,
    )
    ...
    for s in sites_info:
        ...
        calc_ground_load = float(site_load_interp([s["lat"], s["lon"]])[0])

        if s["is_vent"]:
            pts_z.append(s["elev"] + 0.25)
            pts_text.append("VENT")
            pts_hover.append(
                f"<b>Volcanic Vent</b><br>"
                f"Lat: {s['lat']:.4f}°<br>Lon: {s['lon']:.4f}°<br>"
                f"Terrain Elev: {s['elev']:.3f} km"
            )
            pts_color.append("#FF0000")
            pts_symbol.append("diamond")
            pts_size.append(12)
        else:
            csv_load_g = s["csv_load"] * 1000.0 if s["csv_load"] < 1000 else s["csv_load"]
            pts_z.append(s["elev"] + 0.20)
            pts_text.append(s["name"])
            pts_hover.append(
                f"<b>Site: {s['name']}</b><br>"
                f"Lat: {s['lat']:.4f}°<br>Lon: {s['lon']:.4f}°<br>"
                f"Terrain Elev: {s['elev']:.3f} km<br>"
                f"<b>Computed Ground Ash Fall:</b> {calc_ground_load:.2f} g/m²<br>"
                f"<b>CSV Recorded Load:</b> {csv_load_g:.2f} g/m² ({s['csv_load']:.4f} kg/m²)"
            )
...
# ===================================================================
# SECTION 6: SCENE CONSTRUCTION & RENDER ANIMATION
# ===================================================================
fig = go.Figure()
...
initial_ash, initial_z, initial_ground_masked, initial_ground_full = plume_frames_data[0]
...
fig.add_trace(
    go.Surface(
        ...
        surfacecolor=initial_ground_masked,
...
# Umbrella Cloud / Atmospheric Plume
fig.add_trace(
    go.Surface(
        x=X_lon,
        y=Y_lat,
...
initial_site_trace = build_site_trace(initial_ground_full)
fig.add_trace(initial_site_trace)

plotly_frames = []
for idx, (t, (ash_data, z_plume, ground_masked, ground_full)) in enumerate(
    ...
    dynamic_site_trace = build_site_trace(ground_full)
    frame_traces.append(dynamic_site_trace)
...
html_file = outpath / "ash_composite_v2.html"
